...
class HomeController extends Controller
{
    public function index(Request $request)
    {
        ...
        $categories_iphone = Category::where('name', 'like', '%iPhone%')
            ->where('name', 'not like', '%Series%')
            ->inRandomOrder()
            ->take(10)
            ->get();

        $categories_samsung = Category::where(function ($query) {
            $query->where('name', 'like', '%Note%')
        ...
        $featuredPhones = Phone::where('is_active', true)
            ...
            ->with(['variants' => function ($query) {
                $query->where('status', 'còn_hàng') // Chỉ load các biến thể còn hàng lên view
        ...
        // Trong Controller
        $iphones = Phone::with('category')->where('is_active', true)
        ...
        $samsungs = Phone::with('category')->where('is_active', true)
            ->whereHas('category', function ($q) {
